...
- **Light TX queue + GATT write type**: every `0x33` write (headlight, all-on/off, RGB color) goes through a serial queue that spaces frames **≥150 ms** apart (the controller's serial buffer drops back-to-back frames), and is sent **with GATT response** (`writeValueWithResponse`, ATT Write Request) — matching the APK's `BleEcoService.addWriteData()` write queue and Android's WRITE_TYPE_DEFAULT. If the controller rejects the ATT write request, the frame is retried once without response. All other commands (handshake, gears, etc.) keep write-without-response.
